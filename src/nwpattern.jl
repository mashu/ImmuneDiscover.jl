module nwpattern
    ...
    """
        find_gene_unique_kmers(kmers, genomic_hashmap, n; max_combinations=100)

    Find the combination of n kmers such that the combination is not present in the outgroup list
    """
    function find_gene_unique_kmers(kmers, genomic_hashmap, n; max_combinations=100)
        indices = Set{Int}()
        used_combinations = Set()
        comb_vector = shuffle(collect(combinations(first(kmers, n^2), n)))
        for comb in first(comb_vector, max_combinations)
            for i in intersect([get(genomic_hashmap, c, "") for c in comb]...)
                push!(indices, i)
    ...
    function discover_alleles(tsv_path, fasta_path, candidates_tsv, filtered_candidates_tsv; k=7, n=3, max_distance=10, flank_length=21, min_length=200, min_count=5, min_frequency=0.1)
        ...
        for gene in unique(db_genes)
            ...
            indices, used_comb = find_gene_unique_kmers(kmers, genomic_hashmap, n)
            @info "Gene: $gene, Unique kmers: $(length(indices)), kmer combinations: $(length(used_comb))"
